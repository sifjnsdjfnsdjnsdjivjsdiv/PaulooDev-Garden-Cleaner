                ...
                if distance < Config.Job.CarControl.DrawDistance-2 then
                    ...
                    if IsControlJustReleased(0,46) then
                        ...
                        if NumberToDo == 0 then
                            ...
                            TriggerServerEvent("pauloo_gardencleaner:givemoney", JobsToDo, SetRandomLocation)
                        end
                        CarParked = false
                        HasJobStarted = false
...
RegisterNetEvent('pauloo_gardencleaner:startjob')
AddEventHandler('pauloo_gardencleaner:startjob', function()
...
RegisterNetEvent('pauloo_gardencleaner:charvest')
AddEventHandler('pauloo_gardencleaner:charvest', function(data)
...
RegisterNetEvent('pauloo_gardencleaner:LeafBlower')
AddEventHandler('pauloo_gardencleaner:LeafBlower', function(data)
    ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('pauloo_gardencleaner:collecttrash')
AddEventHandler('pauloo_gardencleaner:collecttrash', function(data)
    ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('pauloo_gardencleaner:bin')
AddEventHandler('pauloo_gardencleaner:bin', function(data)
    haveBinBag = false
    POINTS_DONE_IN_JOB = POINTS_DONE_IN_JOB+1
...
RegisterNetEvent("pauloo_gardencleaner:putonclothes")
AddEventHandler("pauloo_gardencleaner:putonclothes", function()
    if ClothesOn then
        ChangeClothes()
    else
        ChangeClothes('work')
...
function SpawnStartingPed()
    ...
    exports[Config.TargetName]:AddTargetModel({GetHashKey('prop_dumpster_01a'), GetHashKey('prop_dumpster_02b'), GetHashKey('prop_dumpster_02a'), GetHashKey('prop_bin_08a')}, {
        options = {
            {
                event = "pauloo_gardencleaner:bin",
    ...
    if not Config.job then
        exports[Config.TargetName]:AddTargetEntity(spawnedPed, {
            options = {
                {
                    event = "pauloo_gardencleaner:startjob",
                    icon = Config.StartJobEmote,
                    label = Config.Languages[Config.Lang]["TARGET_STARTJOB"],
                },
                {
                    event = "pauloo_gardencleaner:putonclothes",
...
function SpawnLeafs()
    ...
    exports[Config.TargetName]:AddTargetEntity(p, {
        options = {
            {
                event = "pauloo_gardencleaner:LeafBlower",
                icon = Config.StartJobEmote,
...
function SpawnCharvest()
        ...
        exports[Config.TargetName]:AddTargetEntity(p, {
            options = {
                {
                    event = "pauloo_gardencleaner:charvest",
                    icon = Config.StartJobEmote,
...
function SpawnBinBags()
    ...
    exports[Config.TargetName]:AddTargetEntity(p, {
        options = {
            {
                event = "pauloo_gardencleaner:collecttrash",
                icon = Config.StartJobEmote,
